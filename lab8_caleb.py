...
def remove_duplicates_set(s):
    """
    This function removes duplicates from the string s.
...
def gem_counting(stones, gems):
    """
    This function counts number of gems in stones.
...
def students_id(ids):
    """
    This function counts the number of unique IDs.
...
def students_id_occurrences(ids):
    """
    This function counts how many times each ID has rode the shuttle.
...
def word_frequency(paragraph):
    """
    This function counts how many times each word appears in paragraph
...
def id_name_repo_starred_300k(response_dict):
    id_name = {}
    for i in range(response_dict["total_count"]):
        id_name[response_dict["items"][i]["id"]] = response_dict["items"][i]["name"]
        # print(response_dict["items"][i]["id"])
        # print(response_dict["items"][i]["name"])
    return id_name
