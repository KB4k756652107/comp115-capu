...
def ask_day_and_trip():
    response_day = input("Input today's day: ")
    response_trip = input("Input the trip duration: ")
    if (response_day.isdigit()) and (response_trip.isdigit()) and (int(response_day) > -1) and (int(response_day) < 7):
        return int(response_day), int(response_trip), True
    return 0, 0, False
