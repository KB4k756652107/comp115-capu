...
def temp_category(temps):
    temps = [temp//15 for temp in temps]
    return [temps.count(2-i) for i in range(3)]

assert temp_category([32, 28, 15, 12, 35]) == [2, 2, 1]
assert temp_category([10, 5, 0]) == [0, 0, 3]
assert temp_category([20, 25, 30]) == [1, 2, 0]

#It works for these number sets specifically, but loses information with anything higher than 44
...
def reverse_list(nums):
    return [nums[index] for index in range(-1, (-(len(nums)) - 1), -1)]
    #return [nums[(index+1)*-1] for index in range(len(nums))]          #This also works
...
assert remove_duplicates([1, 3, 5, 6, 5, 3, 6, 8, 9, 4]) == [1, 3, 4, 5, 6, 8, 9] #added myself
